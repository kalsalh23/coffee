...
export default function OrderDetailPage() {
    ...
    return (
      <main className="mx-auto max-w-lg px-4 pb-4">
        ...
        <div className="mt-4 space-y-3" aria-busy="true">
          <div className="h-36 rounded-3xl bg-brand-100 animate-pulse" />
          <div className="h-48 rounded-3xl bg-brand-100 animate-pulse" />
        </div>
      </main>
    );
  }
      ...
      <main className="mx-auto max-w-lg px-4 pb-4">
        ...
        <div className="mt-16 text-center">
          <div className="w-20 h-20 mx-auto rounded-full bg-brand-100 flex items-center justify-center">
            <ReceiptIcon className="w-9 h-9 text-brand-600" />
          </div>
  ...
  return (
    <main className="mx-auto max-w-lg px-4 pb-4">
      ...
      <section className="mt-2 bg-white border border-brand-100 rounded-3xl p-5">
        ...
        {cancelled ? (
          ...
          <div className="mt-6">
            <div className="relative flex justify-between">
              <div className="absolute top-3.5 inset-x-4 h-0.5 bg-brand-100">
                <div
                  ...
                  <div key={label} className="relative z-10 flex flex-col items-center gap-2 w-16">
                    ...
                    <span
                      className={`text-[10px] font-bold text-center leading-tight ${
                        done ? "text-brand-800" : "text-brand-800/40"
                      }`}
                    >
                      {label}
                    </span>
            ...
            <p className="mt-5 text-center text-sm font-extrabold text-brand-700">
      ...
      <section className="mt-4 bg-white border border-brand-100 rounded-3xl p-4">
        <h2 className="font-black text-sm mb-3">الأصناف</h2>
        <div className="space-y-3">
          {items.map((i) => (
            <div key={i.id} className="flex items-center gap-3">
              <div className="relative w-12 h-12 shrink-0 rounded-2xl bg-gradient-to-br from-brand-100 to-brand-200 flex items-center justify-center text-2xl overflow-hidden">
                {"☕"}
                {i.image_url && (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] font-semibold text-brand-800/60 mt-0.5">
                  {[i.size, ...(i.extras ?? []).map((e) => e.label)].filter(Boolean).join(" • ") ||
                    "بدون إضافات"}
                </p>
              </div>
              <span className="font-bold text-sm text-brand-700 whitespace-nowrap">
                {formatPrice(Number(i.line_total))}
              </span>
            </div>
          ))}
        </div>
        <div className="border-t border-dashed border-brand-200 mt-4 pt-3 space-y-1.5">
      ...
      <section className="mt-4 bg-white border border-brand-100 rounded-3xl p-4 space-y-2.5 text-sm">
        <h2 className="font-black text-sm mb-1">بيانات الطلب</h2>
        <div className="flex items-center gap-2.5 font-semibold text-brand-900/80">
          {order.order_type === "delivery" ? (
            <MapPinIcon className="w-4 h-4 text-brand-600" />
          ) : (
            <BagIcon className="w-4 h-4 text-brand-600" />
          )}
        ...
        <div className="flex items-center gap-2.5 font-semibold text-brand-900/80">
          ...
          <span className="text-brand-800/50">— {order.customer_name}</span>
        </div>
        {order.notes && (
          <p className="text-xs font-semibold text-brand-800/60 bg-brand-50 rounded-xl p-3">
            ملاحظات: {order.notes}
          </p>
        )}
      </section>
      ...
      <div className="fixed bottom-0 inset-x-0 z-40">
        <div className="mx-auto max-w-lg px-4 pb-[calc(env(safe-area-inset-bottom)+14px)] pt-2">
          <Link
            href="/menu"
            className="block text-center rounded-3xl bg-brand-900 text-brand-50 shadow-xl shadow-brand-950/25 py-4 font-extrabold text-sm active:scale-[.98] transition-transform"
          >
            طلب مرة ثانية
          </Link>
        </div>
